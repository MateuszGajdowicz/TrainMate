...
function ChartsPanel({ activitesList, trainingOptions }) {
  ...
  function FormatActivities(activitiesList, activitiesGoal) {
    let displayedChartData = activitiesList.map(element => {
      ...
      return {
        date: element.activityDate,
        value: Number(finalGoal),
        activityType: element.activityType,
        estimatedCalories: element.estimatedCalories,
      }
    })
  ...
  function getSortedByData(analyzedActivitesList, radioDataValue, standardPeriod, periodStart, periodEnd) {
    ...
    let timeFilteredActivities = analyzedActivitesList.filter(element => {
      if (radioDataValue === 'standard') {
        ...
        return (
          new Date(element.activityDate) >= new Date(periodStart) &&
          new Date(element.activityDate) <= new Date(periodEnd)
        )
      }
    })
  ...
  return (
    <>
      ...
      <ActivityToTimeChart
        getUnit={getUnit}
        getSortedByData={getSortedByData}
        FormatActivities={FormatActivities}
        displayedChartData={displayedChartData}
        setDisplayedChartData={setDisplayedChartData}
